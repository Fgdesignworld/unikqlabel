...
import {
  Star, ShoppingBag, Check, Leaf, Circle,
  ...
  FlameKindling, Sparkles, BadgeCheck,
} from "lucide-react"
...
const FEATURES = [
  { icon: Leaf,         label: '100% Natural',       color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
  { icon: FlameKindling,label: 'Traditional Recipe',  color: 'text-orange-400',  bg: 'bg-orange-500/10  border-orange-500/20'  },
  { icon: BadgeCheck,   label: 'No Preservatives',    color: 'text-sky-400',     bg: 'bg-sky-500/10     border-sky-500/20'     },
  { icon: Sparkles,     label: 'Freshly Packed',      color: 'text-purple-400',  bg: 'bg-purple-500/10  border-purple-500/20'  },
  { icon: Award,        label: 'Homemade Quality',    color: 'text-amber-400',   bg: 'bg-amber-500/10   border-amber-500/20'   },
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    productService.getApiBySlug(slug)
      .then(raw => {
        ...
        // Set numeric ID for reviews immediately — no extra round-trip needed
        ...
        const mapped = productService.mapApiProduct ? productService.mapApiProduct(raw) : {
          ...
          rating: raw.rating, bestseller: raw.bestseller, isVeg: raw.is_veg,
          isHomemade: raw.is_homemade, variants: raw.variants || undefined,
    ...
    return (
      <main className="min-h-screen bg-[#0f0f0f]">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-4">
          ...
          <h1 className="text-2xl font-bold text-[#fef3e2]">Product not found</h1>
          <Link to="/products" className="flex items-center gap-2 px-6 py-3 bg-[#d97706] text-[#0f0f0f] font-bold rounded-full hover:bg-[#f59e0b] transition-colors">
  ...
  const galleryImages = [mainImage, ...(Array.isArray(product.gallery) && product.gallery.length > 0 ? product.gallery : [])]
  ...
  const discountPct  = showDiscount ? Math.round(((product.price - product.discount_price!) / product.price) * 100) : 0
  ...
  const doAddToCart = (qty = quantity) => {
    for (let i = 0; i < qty; i++) {
      addItem({
        id: `${product.id}-${variants[selectedVariant].weight}`,
        name: product.name, price: currentPrice,
  ...
  return (
    <main className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="container mx-auto max-w-7xl px-4 pt-24 pb-10">
        ...
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-[#fef3e2]/40 mb-8">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#fef3e2]/70 capitalize">{product.category}</span>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[#fef3e2]">{product.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div className="space-y-4 lg:sticky lg:top-28 lg:self-start">
            <div
              className="relative h-72 sm:h-[400px] lg:h-[480px] rounded-3xl overflow-hidden bg-[#111] border border-[#d97706]/10 cursor-zoom-in group"
                ...
                {galleryImages.map((img, idx) => (
                  <button key={idx} onClick={() => setActiveImg(idx)}
                    className={cn(
                      ...
                      activeImg === idx ? 'border-[#d97706] shadow-lg shadow-[#d97706]/20' : 'border-white/10 opacity-60 hover:opacity-100'
                    )}>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {product.isVeg !== undefined && (
                <span className={cn('flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider',
                  product.isVeg ? 'bg-emerald-900/60 text-emerald-400 border border-emerald-500/30' : 'bg-red-900/60 text-red-400 border border-red-500/30')}>
                  {product.isVeg ? <Leaf className="w-3 h-3 fill-current" /> : <Circle className="w-3 h-3 fill-current" />}
                  {product.isVeg ? 'Pure Veg' : 'Non-Veg'}
                </span>
              )}
              {product.isHomemade && (
                <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-[#451a03]/80 text-[#d97706] border border-[#d97706]/30">
                  Homemade
                </span>
              )}
            </div>
            ...
            {/* Name + Rating */}
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-[#fef3e2] font-serif leading-tight">{product.name}</h1>
              {product.rating && product.rating > 0 ? (
                <div className="flex items-center gap-2 mt-2">
                  <StarRow rating={product.rating} />
                  <span className="text-[#f59e0b] font-bold text-sm">{product.rating}</span>
            ...
            <div className="flex items-baseline gap-3">
              ...
              {showDiscount && (
                <>
                  <p className="text-xl text-[#fef3e2]/30 line-through">{currency}{product.price.toLocaleString('en-IN')}</p>
            ...
            <div className="flex items-center gap-4 text-xs text-[#fef3e2]/40">
              <span className="flex items-center gap-1.5"><Truck className="w-3.5 h-3.5" /> Delivery in 2–3 business days</span>
              <span className="flex items-center gap-1.5"><Package className="w-3.5 h-3.5" /> Freshly packed</span>
            </div>

            {/* Description (short) */}
            {product.description && (
              <p className="text-[#fef3e2]/70 leading-relaxed text-sm border-l-2 border-[#d97706]/40 pl-4">
            ...
            {variants.length > 1 && (
              <div>
                <p className="text-sm font-semibold text-[#fef3e2]/50 uppercase tracking-widest mb-3">Select Size</p>
                <div className="flex flex-wrap gap-3">
                  {variants.map((v, idx) => (
                    <button key={v.weight} onClick={() => setSelectedVariant(idx)}
                      className={cn('px-5 py-3 rounded-xl text-sm font-bold transition-all',
                        selectedVariant === idx
                          ? 'bg-[#d97706] text-[#0f0f0f] shadow-lg shadow-[#d97706]/20'
                          : 'bg-[#1a1410] text-[#fef3e2]/70 border border-[#d97706]/20 hover:border-[#d97706]/60')}>
                      {v.weight}
                      <span className={cn('block text-xs mt-0.5', selectedVariant === idx ? 'text-[#0f0f0f]/70' : 'text-[#d97706]')}>
                        {currency}{v.price.toLocaleString('en-IN')}
                      </span>
                    </button>
                  ))}
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-[#111] border border-gray-800 rounded-2xl p-5">
                      ...
                      <ul className="space-y-1.5 text-gray-400 text-xs leading-relaxed">
                        ...
                        <li>• Refrigerate after opening for freshness</li>
                        <li>• Consume within 3–6 months of opening</li>
                      </ul>
                    </div>
                    <div className="bg-[#111] border border-gray-800 rounded-2xl p-5">
                      <p className="text-amber-400 font-black text-sm mb-3 flex items-center gap-2">
                        <Award className="w-4 h-4" /> Why Laxmi Home Foods?
                      </p>
                      <ul className="space-y-1.5 text-gray-400 text-xs leading-relaxed">
                        <li>• Made fresh in small batches</li>
                        <li>• Zero artificial colours or flavours</li>
                        <li>• Authentic Andhra recipes</li>
                        <li>• Packed with hygiene &amp; care</li>
                      </ul>
                    </div>
                  </div>
                </div>
        ...
        <section className="mt-14">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              { icon: ShieldCheck,title: 'Quality Guaranteed',  desc: '100% natural, no artificial additives' },
              { icon: Package,    title: 'Freshly Packed',      desc: 'Packed to order, never pre-stocked' },
            ].map(item => (
      ...
      {/* ── Mobile Sticky CTA ─────────────────────────────────────── */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0a0a0a]/95 backdrop-blur-xl border-t border-gray-800 px-4 py-3 flex gap-3">
        <button onClick={handleAddToCart} disabled={isAdded}
          className={cn('flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm transition-all',
            isAdded ? 'bg-green-600 text-white' : 'bg-[#d97706] text-black')}>
          {isAdded ? <><Check className="w-4 h-4" /> Added!</> : <><ShoppingBag className="w-4 h-4" /> Add to Cart</>}
        </button>
        <button onClick={handleBuyNow}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-black text-sm border border-[#d97706]/40 text-[#d97706]">
