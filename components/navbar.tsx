      ...
      >
        <div className="w-full max-w-7xl mx-auto px-3 flex items-center justify-between">
          <nav className="flex items-center justify-between w-full">
            ...
            <div className="hidden lg:flex items-center bg-white/5 backdrop-blur-md rounded-full px-6 py-1.5 border border-white/10 gap-4 absolute left-1/2 transform -translate-x-1/2">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.name}
                    to={link.href}
                    className={`text-[12px] uppercase tracking-[0.12em] font-medium transition-all hover:text-[#f59e0b] relative group ${isActive ? "text-[#d97706]" : "text-[#fef3e2]/70"
                      }`}
                  >
                    {link.name}
